  ...
  for(i_out=0;i_out<FC1_NBOUTPUT;i_out++){
    ...
    for(i_in=0;i_in<POOL2_NBOUTPUT;i_in++){
      ...
      for(j=0;j<POOL2_HEIGHT;j++){
		...
		#pragma HLS RESOURCE variable=tmp core=MulnS latency=2

	        tmp=tmp+input[i_in][j][k]*kernel[i_out][i_in][j][k];
	      }
      }
    }
...
void Softmax(short vector_in[FC2_NBOUTPUT], float vector_out[FC2_NBOUTPUT]){
  ...
  for(i=0;i<FC2_NBOUTPUT;i++){
    ...
    sum = sum + expf((double)vector_out[i]/(double)(1<<FIXED_POINT));
  }
  for(i=0;i<FC2_NBOUTPUT;i++){
