...
void RescaleImg(unsigned char *input, short width,short height, short *output, short new_width, short new_height) {
  short x, y; 
...
void NormalizeImg(unsigned char *input, short *output, short width, short height) {
  ...
  for (y=0; y<height; y++) 
    for (x=0; x<width; x++) 
      ...
      output[(y*width)+x] = (short)( (float)input[(y*width)+x] * (1<<FIXED_POINT) / 255); 
    ...
    for (y = 0; y < CONV1_DIM; y++)
      for (x = 0; x < CONV1_DIM; x++) 
		for (z = 0; z < IMG_DEPTH; z++) 
		  weight[k][z][y][x] = (short) (buffer_float[y][x][z][k] * (1<<FIXED_POINT)); // re-ordering [y][x][z][k] -> [k][z][y][x]
...
void ReadConv1Bias(char *filename, char *datasetname, short *bias) {
  ...
  status = H5Dread (dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buffer_float); 
  for (k = 0; k < CONV1_NBOUTPUT; k++) 
    bias[k] = (short)(buffer_float[k] * (1<<FIXED_POINT)); 
    ...
    for (z = 0; z < POOL2_NBOUTPUT; z++)
      for (y = 0; y < POOL2_HEIGHT; y++)
        for (x = 0; x < POOL2_WIDTH; x++)
		  ...
		  weight[k][z][y][x] = (short)(buffer_float[(y*POOL2_WIDTH*POOL2_NBOUTPUT)+(x*POOL2_NBOUTPUT)+z][k] *(1<<FIXED_POINT)); // re-ordering [yxz][k] -> [k][z][y][x]
...
void ReadFc2Weights(char *filename, char *datasetname, short weight[FC2_NBOUTPUT][FC1_NBOUTPUT]) {
  ...
  for (k = 0; k < FC2_NBOUTPUT; k++)
    for (z = 0; z < FC1_NBOUTPUT; z++)
	  weight[k][z] = (short)(buffer_float[z][k] *(1<<FIXED_POINT)); // re-ordering [z][k] -> [k][z]
